  ...
  )
{
  ...
  DEBUG ((DEBUG_ERROR, "UsbRndisDriverStart %x\n", ControllerHandle));
  ...
  if (UsbRndisDevice->UsbIoCdcData) {
    ...
    DEBUG ((DEBUG_ERROR, "Rndis DeviceHandle %r\n", UsbRndisDevice->UsbRndisHandle));
    DEBUG ((DEBUG_ERROR, "CDC DeviceHandle %r\n", UsbRndisDevice->UsbCdcDataHandle));
    return EFI_SUCCESS;
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_ERROR, "UsbRndisDriverStop ControllerHandle %lx\n", ControllerHandle));
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "CheckandStopRndisDevice %r\n", Status));
  ...
  DEBUG ((DEBUG_ERROR, "UsbRndisDriverStop %r\n", Status));
  return Status;
}
