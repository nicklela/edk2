  ...
  )
{
  ...
  UINTN                       RetryCount;
  REDFISH_REQUEST             RequestHeaders;
  ...
  ZeroMem (&RequestHeaders, sizeof (REDFISH_REQUEST));
  ...
  //
  // If additional request data is provided, pass them to
  // RedfishLib.
  //
  if (Request != NULL) {
    RequestHeaders.HeaderCount = Request->HeaderCount;
    RequestHeaders.Headers     = Request->Headers;
    RequestHeaders.Context     = Request->Context;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "%a: patch %a failed (%d/%d): %r\n", __func__, AsciiUri, RetryCount, Private->RetrySetting.MaximumRetryPatch, Status));
    goto ON_RELEASE;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "%a: put %a failed (%d/%d): %r\n", __func__, AsciiUri, RetryCount, Private->RetrySetting.MaximumRetryPut, Status));
    goto ON_RELEASE;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "%a: post %a failed (%d/%d): %r\n", __func__, AsciiUri, RetryCount, Private->RetrySetting.MaximumRetryPost, Status));
    goto ON_RELEASE;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_ERROR, "%a: delete %a failed (%d/%d): %r\n", __func__, AsciiUri, RetryCount, Private->RetrySetting.MaximumRetryDelete, Status));
    goto ON_RELEASE;
  }
