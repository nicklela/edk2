...
//  (void)verifyconnect(sockfd, NULL);
  ...
  //
  // Calling non-blocking connect() is a way to update the flags regarding the
  // connection status in sockfd, so Curl_socket_ready() can figure out if the
  // connection was timed out or not.
  //
  c = connect(sockfd, &waddr.sa_addr, waddr.addrlen);
  if (c == 0) {
    //
    ...
    //
    // connect() is not ready to be polled.
    // Wait one second before checking connection status again.
    //
    sleep(1);
  } else {
    //
    // If connect() error is different from EAGAIN or EINPROGRESS, then
    // connect() has failed and there is no reason to keep polling.
    //
    rc = -1;
  }
#else
...
#ifdef _UEFI_
  //
  // It seems that getsockopt in EDK2 (StdLib) does not support SO_ERROR.
  // (The returning status is always EINVAL)
  //
  if (EINVAL == err) {
    SET_SOCKERRNO(0);
    err = 0;
...
#ifdef _UEFI_
  //
  // We save the address information in an auxiliary struct to use it
  // when polling connect() to check connection status.
  //
  memcpy(&waddr.sa_addr, ai->ai_addr, ai->ai_addrlen);
  waddr.addrlen = ai->ai_addrlen;
#endif  // _UEFI_
